...
export async function monitor (srcUrl, { reporter = consoleReporter }) {
    ...
    const readable = new HlsSegmentReadable(new HlsSegmentFetcher(playlistFetcher, { fullStream: true }));
    ...
    const streamer = new HlsSegmentStreamer(readable, { withData: false, highWaterMark: 3 });
    for await (const segmentInfo of streamer) {
