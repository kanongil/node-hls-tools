...
internals.limitedWait = function(waitMs, limit) {

  assert(waitMs <= limit, `Unexpectedly high wait time: ${waitMs}ms`)
...
export class Smoother extends Transform {
  ...
  constructor(options) {
    ...
    if (options?.maxPresentationDelay) {
      this.limits.future = options.maxPresentationDelay;
    }
  }
  ...
  async #transform(chunk) {
    ...
    if (outtime) {
      ...
      await internals.limitedWait(waittime, this.limits.future);
    }
  ...
  async #pushSmoothed(buffers, endTime) {
    ...
    while (index < bl.length) {
      ...
      if (remainingBytes > 0) {
        ...
        if (delay >= 1) {
          await internals.limitedWait(delay, this.limits.future);
          accum = 0;
        }
        else {
          accum += delay;
