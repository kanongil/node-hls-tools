...
export const decrypt = async function (stream, keyAttrs, options) {

    if (!keyAttrs || !options) {
        return stream;
    }

    const key = internals.getIdentityKey(new Set(...methods.keys()), keyAttrs);
    const decryptor = methods.get(key?.method);
    if (!decryptor) {
        return stream;
    }
