      ...
      if (this.subreader) {
        ...
        this.index.variants.forEach((variant, index) => {
          ...
          if (!rec || !rec.localUrl) {
            ...
            rec = new HlsStreamRecorder(this.subreader(variantUrl), new URL(dir, this.dst).href, { startOffset: this.startOffset, collect: this.collect, decrypt: this.decrypt });
        ...
        allGroups.forEach((groupItem, index) => {
          ...
          if (srcUri) {
            ...
            if (!rec || !rec.localUrl) {
              ...
              rec = new HlsStreamRecorder(this.subreader(itemUrl), new URL(dir, this.dst).href, { startOffset: this.startOffset, collect: this.collect, decrypt: this.decrypt });
              rec.localUrl = Url.format({pathname: Path.join(dir, 'index.m3u8')});
