...
import { TsSmooth } from './tssmooth.mjs';
...
export const HlsReader = class extends Readable {
  ...
  hook(startTime) {                          // the hook is used to prebuffer
    ...
    let s = this.buffer;
    if (this.sync) {
      const collector = new TsCollect();
      const smooth = new TsSmooth({ startTime });
      ...
      const reporter = setInterval(() => {
        ...
        if (smooth.lateDropped !== last) {
          console.error(`late bytes dropped: ${smooth.lateDropped - last}`);
          last = smooth.lateDropped;
        }
      }, 1000);

      finished(smooth, () => clearTimeout(reporter));
    }
